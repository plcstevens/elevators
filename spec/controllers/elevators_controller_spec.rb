require 'spec_helper'

# This spec was generated by rspec-rails when you ran the scaffold generator.
# It demonstrates how one might use RSpec to specify the controller code that
# was generated by Rails when you ran the scaffold generator.
#
# It assumes that the implementation code is generated by the rails scaffold
# generator.  If you are using any extension libraries to generate different
# controller code, this generated spec may or may not pass.
#
# It only uses APIs available in rails and/or rspec-rails.  There are a number
# of tools you can use to make these specs even more expressive, but we're
# sticking to rails and rspec-rails APIs to keep things simple and stable.
#
# Compared to earlier versions of this generator, there is very limited use of
# stubs and message expectations in this spec.  Stubs are only used when there
# is no simpler way to get a handle on the object needed for the example.
# Message expectations are only used when there is no simpler way to specify
# that an instance is receiving a specific message.

describe ElevatorsController do

  # This should return the minimal set of attributes required to create a valid
  # Elevator. As you add validations to Elevator, be sure to
  # update the return value of this method accordingly.
  def valid_attributes
    { "elevator_system" => "" }
  end

  # This should return the minimal set of values that should be in the session
  # in order to pass any filters (e.g. authentication) defined in
  # ElevatorsController. Be sure to keep this updated too.
  def valid_session
    {}
  end

  describe "GET index" do
    it "assigns all elevators as @elevators" do
      elevator = Elevator.create! valid_attributes
      get :index, {}, valid_session
      assigns(:elevators).should eq([elevator])
    end
  end

  describe "GET show" do
    it "assigns the requested elevator as @elevator" do
      elevator = Elevator.create! valid_attributes
      get :show, {:id => elevator.to_param}, valid_session
      assigns(:elevator).should eq(elevator)
    end
  end

  describe "GET new" do
    it "assigns a new elevator as @elevator" do
      get :new, {}, valid_session
      assigns(:elevator).should be_a_new(Elevator)
    end
  end

  describe "GET edit" do
    it "assigns the requested elevator as @elevator" do
      elevator = Elevator.create! valid_attributes
      get :edit, {:id => elevator.to_param}, valid_session
      assigns(:elevator).should eq(elevator)
    end
  end

  describe "POST create" do
    describe "with valid params" do
      it "creates a new Elevator" do
        expect {
          post :create, {:elevator => valid_attributes}, valid_session
        }.to change(Elevator, :count).by(1)
      end

      it "assigns a newly created elevator as @elevator" do
        post :create, {:elevator => valid_attributes}, valid_session
        assigns(:elevator).should be_a(Elevator)
        assigns(:elevator).should be_persisted
      end

      it "redirects to the created elevator" do
        post :create, {:elevator => valid_attributes}, valid_session
        response.should redirect_to(Elevator.last)
      end
    end

    describe "with invalid params" do
      it "assigns a newly created but unsaved elevator as @elevator" do
        # Trigger the behavior that occurs when invalid params are submitted
        Elevator.any_instance.stub(:save).and_return(false)
        post :create, {:elevator => { "elevator_system" => "invalid value" }}, valid_session
        assigns(:elevator).should be_a_new(Elevator)
      end

      it "re-renders the 'new' template" do
        # Trigger the behavior that occurs when invalid params are submitted
        Elevator.any_instance.stub(:save).and_return(false)
        post :create, {:elevator => { "elevator_system" => "invalid value" }}, valid_session
        response.should render_template("new")
      end
    end
  end

  describe "PUT update" do
    describe "with valid params" do
      it "updates the requested elevator" do
        elevator = Elevator.create! valid_attributes
        # Assuming there are no other elevators in the database, this
        # specifies that the Elevator created on the previous line
        # receives the :update_attributes message with whatever params are
        # submitted in the request.
        Elevator.any_instance.should_receive(:update_attributes).with({ "elevator_system" => "" })
        put :update, {:id => elevator.to_param, :elevator => { "elevator_system" => "" }}, valid_session
      end

      it "assigns the requested elevator as @elevator" do
        elevator = Elevator.create! valid_attributes
        put :update, {:id => elevator.to_param, :elevator => valid_attributes}, valid_session
        assigns(:elevator).should eq(elevator)
      end

      it "redirects to the elevator" do
        elevator = Elevator.create! valid_attributes
        put :update, {:id => elevator.to_param, :elevator => valid_attributes}, valid_session
        response.should redirect_to(elevator)
      end
    end

    describe "with invalid params" do
      it "assigns the elevator as @elevator" do
        elevator = Elevator.create! valid_attributes
        # Trigger the behavior that occurs when invalid params are submitted
        Elevator.any_instance.stub(:save).and_return(false)
        put :update, {:id => elevator.to_param, :elevator => { "elevator_system" => "invalid value" }}, valid_session
        assigns(:elevator).should eq(elevator)
      end

      it "re-renders the 'edit' template" do
        elevator = Elevator.create! valid_attributes
        # Trigger the behavior that occurs when invalid params are submitted
        Elevator.any_instance.stub(:save).and_return(false)
        put :update, {:id => elevator.to_param, :elevator => { "elevator_system" => "invalid value" }}, valid_session
        response.should render_template("edit")
      end
    end
  end

  describe "DELETE destroy" do
    it "destroys the requested elevator" do
      elevator = Elevator.create! valid_attributes
      expect {
        delete :destroy, {:id => elevator.to_param}, valid_session
      }.to change(Elevator, :count).by(-1)
    end

    it "redirects to the elevators list" do
      elevator = Elevator.create! valid_attributes
      delete :destroy, {:id => elevator.to_param}, valid_session
      response.should redirect_to(elevators_url)
    end
  end

end
